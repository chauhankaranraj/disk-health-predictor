...
class RHDiskHealthClassifier(DiskHealthClassifier):
    ...
    @staticmethod
    def _get_vendor(disk_days: dict) -> Optional[str]:
        ...
        # if not available as smartctl attr then infer using model name
            ...
            if modelname_key_found:
                ...
                vendor = RHDiskHealthClassifier._estimate_vendor_from_model(model_name)
            else:
    ...
    def predict(self, disk_days: Sequence[DevSmartT]) -> str:
        # get vendor
        vendor = RHDiskHealthClassifier._get_vendor(disk_days)
